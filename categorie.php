...
<?php
 if(isset($_POST['add_cat'])){
   ...
   if(empty($errors)){
      $sql  = "INSERT INTO Accounts (account,email,postalAddress)";
      $sql .= " VALUES ('{$cat_name}','{$email}','{$address}')";
      if($db->query($sql)){
        $session->msg("s", "Successfully Added Account");
        ...
        <div class="panel-body">
          <form method="post" action="categorie.php">
            <div class="form-group">
                <input type="text" class="form-control" name="categorie-name" placeholder="Account Name">
                <input type="text" class="form-control" name="email" placeholder="Email">
                <input type="text" class="form-control" name="address" placeholder="postal address">
            </div>
            <button type="submit" name="add_cat" class="btn btn-primary">Add Account</button>
        </form>
        </div>
      </div>
    </div>
            ...
            <tbody>
              <?php foreach ($all_categories as $cat):?>
                <tr>
                    ...
                    <td><?php echo remove_junk(ucfirst($cat['account'])); ?></td>
                    <td><?php echo remove_junk(ucfirst($cat['email'])); ?></td>
                    <td><?php echo remove_junk(ucfirst($cat['postalAddress'])); ?></td>
